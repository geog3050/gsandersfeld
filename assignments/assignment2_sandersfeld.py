...
#
    ...
    while fighter1Health > 0 and fighter2Health > 0: # AND or OR?
        if first2attack == 1:
        ...
        elif first2attack == 2: first2attack == 1
    if fighter1Health > 0:
        winner = 1
    if fighter2Health > 0:
        winner = 2
    ...
    for pk1 in range(0, len(participants)):
        for pk2 in range(pk1, len(participants)):
            if pk1 != pk2:
                ...
                elif fight(participants[pk1], participants[pk2], 2)[0] == 2: # away match
                    wins[pk1] += 1 # increment pokemon's wins index for the away win
    return wins
### the total number of matches is 30 = 2*6choose2 or n(n-1)
