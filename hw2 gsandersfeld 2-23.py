...
def import_data(filename):
    ...
    #           3.2.1. 1st item of the list should be the name of the pokemon (type: str)
    #           3.2.2. 2nd item of the list should be the type of the pokemon (type: str)
    #           3.2.3. 3rd item of the list should be the health (HP) of the pokemon (type: float)
    #           3.2.4. 4th item of the list should be the base attack damage of the pokemon (type: float)
    ...
    ######################
    participants,index = [],0 # initiate empty list for storing participants, and index for line numnber (participant and their stats)
    # my code after this line:
    f = open(filename,'r')
    for line in f:
        index += 1
        stats = line.splitlines()[0]
        pokeName = stats.split(',')[0]
        pokeType = stats.split(',')[1]
        pokeHP = float(stats.split(',')[2])
        pokeBaseAtk = float(stats.split(',')[3])
        participant = [pokeName,pokeType,pokeHP,pokeBaseAtk]
...
def attack_multiplier(attacker_type, defender_type):
    ...
    """
    ##### Pseudocode #####
    # 1. If Attacker (attacker_type) is Water and Defender (defender_type) is Fire, return 2.5
...
def fight(participant1, participant2, first2attack):
    ...
    #    - rounds = 0
    # 2. Store the initial health of both participants in two variables because we don't want to change the base pokemon stats
    #    - variable1 = HP of the first participant (fighter1Health)
    #    - variable2 = HP of the second participant (fighter2Health)
    ...
    # 4. If variable1 (fighter1HP) is greater than 0, then winner is 1. However, if variable 2 (fighter2HP) is greater than 0,
    #    then winner is 2.
    ...
    winner = 1
#
    fighter1Health = participant1[2] # stores the initial health of first participant
    fighter2Health = participant2[2] # stores the initial health of second participant
    while fighter1Health and fighter2Health > 0: # AND or OR?
        if first2attack == 1:
            fighter1Atk = (participant1[3])*(attack_multiplier(participant1[1],participant2[1])) # new attack dmg with multiplier if first participant is first to attack
            fighter2Health -= fighter1Atk
        elif first2attack == 2:
            fighter2Atk = (participant2[3])*(attack_multiplier(participant2[1],participant1[1])) # new attack dmg with multiplier if second participant is first to attack
            fighter1Health -= fighter2Atk
        rounds += 1
        if first2attack == 1: first2attack = 2
        if first2attack == 2: first2attack = 1
    if fighter1Health > 0:
        winner = 1
    if fighter2Health > 0:
        winner = 2
    if winner == 1: print('winner is',participant1[0],'loser is',participant2[0],rounds,"rounds")
    if winner == 2: print('winner is',participant2[0],'loser is',participant1[0],rounds,"roumds")
...
def tournament(participants):
    ...
    3a. Recall, don't hard code your functions
    """
    ##### Pseudocode #####
    ...
    for i in range(0,len(participants)):
        wins.append(0)
#    for pk1 in range(0, len(participants)):
#        for pk2 in range((pk1+1), len(participants)):
#            fight(participants[pk1], participants[pk2], 1) # home game
#            fight(participants[pk1], participants[pk2], 2) # away game


    for pk1 in range(0, len(participants)):
        for pk2 in range(pk1, len(participants)):
            if pk1 != pk2:
                #fight(participants[pk1], participants[pk2], 1) 
                if fight(participants[pk1], participants[pk2], 1)[0] == 1: # home match
                    wins[pk1] += 1 # increment pokemon's wins index for the home win
                #fight(participants[pk1], participants[pk2], 2)
                if fight(participants[pk1], participants[pk2], 2)[0] == 1: # away match
                    wins[pk1] += 1 # increment pokemon's wins index for the away win
    return wins
###
